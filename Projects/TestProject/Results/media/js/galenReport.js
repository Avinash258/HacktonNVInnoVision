...
function setStatusLinkWeb(stat,stepNo,fileLink){
	fileLink=fileLink.replaceAll("\\","/");
	return "<a class='exe table "+ stat.toUpperCase()+ "' onclick='showDialog("+ stepNo +",\""+ fileLink +"\")'>"+ stat +" </a>";
}
...
function showDialog(stepNo, fileLink) {
	var request=fileLink+"/Step-"+stepNo+"_Request.txt";
	var response=fileLink+"/Step-"+stepNo+"_Response.txt";
    showShadow();
	$('#Request').attr("fileName", request);
	$('#Response').attr("fileName", response);
