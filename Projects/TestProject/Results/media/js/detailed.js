...
function renderTableSNGL(data, browser) {
    ...
    function getRowHtm(row) {
		...
		if(stat=="COMPLETE"){
			var statElem = setStatusLinkWeb(stat, row[exeDetails_ID[0]], row['link']);
		} else{
...
function openTab(evt, tabName) 
{
			  ...
			  //$('#Response').load("./web/Step-2_Response.html");
			  //console.log("onclick wala response "+$("#Response").attr("filename"));
			  document.getElementById("Response").innerText=loadFile($("#Response").attr("filename"));
			  document.getElementById("Request").innerText=loadFile($("#Request").attr("filename"));
			  //console.log("onclick wala request "+$("#Request").attr("filename"));
}
			
function loadFile(fileLoc){
	var text;
	/*
	fs.read(fileLoc,"utf-8", (err, text) => {
    if (err) throw err;
